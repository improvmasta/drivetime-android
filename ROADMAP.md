...
- [built] **Tracker settings live in the SPA's Settings tabs** — General/Tracking/Devices/Sync/More, reached in one tap from the shell's status pill (`#/settings`). Every routine-controllable knob (`Control.SET_KEYS`) plus server/backup/updates is a real in-tab web control that reads/writes over the `DrivetimeNative` bridge (`getSettings`/`getStatus`/`setSetting`, values shared with the routine API via `Control.set`). The genuinely-native flows a WebView can't do — permission prompts, Bluetooth/OBD pairing, backup file-pickers, the QR pairing scanner, the APK updater, Test connection — are hosted by `WebViewActivity` and fired *in place* from their tab (bridge actions like `requestPermission`/`pickBluetooth`/`scanPairingQr`/`checkForUpdate`). The old separate native `LoggerActivity`/`SettingsActivity` screen is retired. Sync is genuinely optional: tracking runs standalone with no server (STANDALONE.md).
